...
> **Septiembre 2026 — migración fuera de Supabase.** El proyecto vivía en Supabase (Postgres + Storage + Auth), pero ocupaba uno de los dos proyectos gratuitos de esa cuenta para una base de datos mínima (4 eventos, 1 redirect). Se migró a un API propio en PHP/MySQL sobre el hosting compartido de Hostinger que ya se pagaba por otro motivo, y se dio de baja el proyecto de Supabase. Detalles de la decisión y el diseño en §4. El repositorio ya no conserva `supabase/migrations/`; el esquema final que documentaban esas migraciones quedó traducido a MySQL en `hostinger-api/schema.sql`, y el resumen narrativo vive abajo en §2 y §4.
...
**Historia del modelo de datos** (antes vivía en `supabase/migrations/`, ya no está en el repo): `events` reemplazó el par ambiguo `title`/`subtitle` por el esqueleto fijo descrito en §3.2, se agregaron `redirects`+`qr_scans` para el QR, se eliminó la tabla `hero_templates` (portada compuesta desde `events`, ver §3.3), y se agregó `slug` para URL propia por evento (§3.5). `hostinger-api/schema.sql` es la traducción a MySQL de ese estado final — es, hoy, la única fuente de verdad del esquema.
...
- **Por qué se migró de Supabase a un API propio en Hostinger (septiembre 2026):** la base de datos era mínima (4 eventos, 1 redirect, un puñado de escaneos de QR) y ocupaba uno de los dos proyectos gratuitos de la cuenta de Supabase del dueño del sitio — no se justificaba seguir usando ese cupo para este tamaño de datos, cuando ya se paga hosting compartido en Hostinger que puede correr esto sin costo adicional. La app en Next.js se quedó igual en Vercel; solo cambió de dónde vienen los datos. Los 4 eventos que había en Supabase ya habían pasado para cuando se hizo el corte, así que se decidió no migrar sus imágenes a Hostinger — sus tres campos `imagen_*` quedaron apuntando a Supabase Storage, y dejarán de cargar en cuanto se borre ese proyecto. Como `next.config.ts` ya no permite ese dominio, esas miniaturas se van a ver rotas en la lista de `/admin/agenda` (no en el sitio público, que nunca muestra eventos pasados) — es cosmético y esperado; se resuelve solo borrando esos 4 eventos viejos desde el admin cuando estorben.
